    ...
    try {
      ...
      client.stdout?.on("data", (data) => {
        process.stdout.write(
          data
            .split("\n")
            .map((line: string) => (line ? `${CLIENT_TAG}  ${line.trimEnd()}\n` : ""))
            .join("")
        );
      });
      ...
      server.on("stdout", (data) => {
        process.stdout.write(
          data
            ...
            .trim()
            .split("\n")
            .map((line: string) => (line ? `${SERVER_TAG}  ${line.trimEnd()}\n` : ""))
            .join("")
        );
      });
...
function gracefulShutdown(...processes: Array<ChildProcess>) {
  return () => {
