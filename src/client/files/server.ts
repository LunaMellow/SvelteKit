import type { HttpRequest, HttpResponse, Server, ServerNext } from "../../server";
...
const noop_handler = (req: HttpRequest, res: HttpResponse, next: ServerNext) => next();
const paths = {
  assets: join(config.outDir, "/assets"),
  prerendered: join(config.outDir, "/prerendered"),
};

const immutable_path = (pathname: string) => {
  // eslint-disable-next-line
  // @ts-ignore
  let app_dir = "_app";
...
// @ts-ignore
// eslint-disable-next-line
export function getServer({ render } = { render: () => {} }): Server {
  ...
  const server = polka({ server: httpServer, onNoMatch: onNoMatchHandler(render) }).use(
