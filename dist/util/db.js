    ...
    for (const envKey in process.env) {
        if (envKey.startsWith(DB_URI_PREFIX)) {
            ...
            db[dbName.toLowerCase()] = knex({
                ...
                migrations: {
                    directory: `${process.env.NODE_ENV === "development" ? config.rootDir : config.outDir}/db/migrate/${dbName.toLowerCase()}`,
                    extension,
                    tableName: "schema_migrations",
                    loadExtensions,
                },
                seeds: {
                    directory: `${process.env.NODE_ENV === "development" ? config.rootDir : config.outDir}/db/seed/${dbName.toLowerCase()}`,
                    extension,
                    loadExtensions,
                },
