...
cmd
    .command("config:secret:rotate <oldMasterKey>", "Decrypt the encrypted config with the old master key and re-encrypt with newly generated master key. (only for NODE_ENV=development)")
    .action((oldMasterKey) => {
    try {
        ...
        for (const k of Object.keys(parsedConfig).sort()) {
            if (parsedConfig[k].trim().endsWith(CONFIG_ENC_SUFFIX)) {
                const decryptedText = decrypt(oldMasterKey, parsedConfig[k].replace(CONFIG_ENC_SUFFIX, "").trim());
        ...
        console.log(`Successfully updated '${configPath}' with the new master key ${colors.bold.whiteBright.bgGreen(newMasterKey)}!`);
    }
    catch (err) {
        logger.error(err);
        process.exit(1);
